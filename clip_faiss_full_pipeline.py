...
# ==== CONFIG ====
IMAGE_DIR = "designs"
OUTPUT_CSV = "clip_nn_similar.csv"
OUTPUT_HTML = "clip_nn_visualizer.html"
TOP_K = 5
...
# ==== SIMILARITY SEARCH ====
print("🔍 Searching using Nearest Neighbors...")
...
with open(OUTPUT_CSV, "w") as f:
    f.write("Query,Similar,Similarity\n")
    for query_idx, neighbors in enumerate(indices):
        query_img = valid_paths[query_idx].name
        for rank, idx in enumerate(neighbors[1:], start=1):
            similarity = 1 - distances[query_idx][rank]
            if similarity < SIMILARITY_THRESHOLD:
                continue
            similar_img = valid_paths[idx].name
            f.write(f"{query_img},{similar_img},{similarity:.4f}\n")
...
""")
    ...
    for query_idx, neighbors in enumerate(indices):
        ...
        # Query image (styled like the others)
